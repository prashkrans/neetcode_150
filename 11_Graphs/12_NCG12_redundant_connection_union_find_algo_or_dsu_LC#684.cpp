...
class Solution {
    public:
        ...
        void unionOfSets(int u, int v, unordered_map<int, pair<int, int>> &subset) {
            ...
            // auto [uRoot, uRank] = findSet(u, subset);
            // if(vRank <= uRank) {
...
class Solution {
    public:
        ...
        int findSet(int u, vector<pair<int, int>> &subset) { // findSet recursive does full path compression
            // int parent = subset[u].first;
            // if(u == parent) return subset[u].first;
            // else
            //     return subset[u].first = findSet(parent, subset); 
            while(true) {   // findSet iterative doesn't do full path compression thus is inefficient
                int parent = subset[u].first;
                if(u == parent) break;
                else {
                    subset[u].first = subset[parent].first;
                    u = parent;
                }
            }
        ...
        void unionOfSets(int u, int v, vector<pair<int, int>> &subset) {
            ...
            if(uHeight <= vHeight) {    // Three cases in a compact way
