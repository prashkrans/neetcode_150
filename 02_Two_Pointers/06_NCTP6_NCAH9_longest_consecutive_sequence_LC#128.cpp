...
// 1. Using an unordered hash set with linear sequence building
...
public:
    int longestConsecutive(vector<int>& nums) {
        ...
        for(auto i: st) {                                        // Iterate through each unique element
            if(st.find(i-1) == st.end()) {                       // Check if this is the start of a sequence
...
public:
   int longestConsecutive(vector<int>& nums) {
       ...
       vector<int> newNums;                                   // Vector to store sortedn and only unique elements
